...
mod tests {
    ...
    fn test_duration_to_blocks() {
        // (offset, block_time, expected)
        let cases = [
            (Duration::ZERO, 2, 0),
            (Duration::from_secs(3600), 0, 0),
            (Duration::from_secs(3), 2, 2),
            (Duration::from_secs(4), 2, 2),
            (Duration::from_secs(15), 4, 4),
            (Duration::from_secs(16), 4, 4),
            (Duration::from_millis(500), 1, 0),
            (Duration::from_secs(12 * 3600), 2, 21600),
        ];
        for (offset, block_time, expected) in cases {
            assert_eq!(
                duration_to_blocks(offset, block_time),
                expected,
                "offset={offset:?} block_time={block_time}"
            );
        }
    }
    ...
    fn test_offset_block_num() {
        // (offset, block_time, head, genesis, expected)
        let cases = [
            (Duration::ZERO, 2, 100, 0, 100),
            (Duration::from_secs(10), 2, 0, 0, 0),
            (Duration::from_secs(10), 2, 5, 10, 5),
            (Duration::from_secs(10), 2, 100, 0, 95),
            (Duration::from_secs(1000 * 3600), 2, 10, 0, 0),
            (Duration::from_secs(10), 2, 15, 10, 10),
            (Duration::from_secs(15), 4, 100, 0, 96),
        ];
        for (offset, block_time, head, genesis, expected) in cases {
            assert_eq!(
                offset_block_num(offset, block_time, head, genesis),
                expected,
                "offset={offset:?} block_time={block_time} head={head} genesis={genesis}"
            );
        }
    }
